...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  Future<void> chatNewPromptEvent(
    ...
    try {
      ...
      cachedMessage.add(ChatMessageModel(role: 'assistant', content: ""));
      _subscription?.cancel();
      _subscription = ChatRepo.getChatGptResponseRepo(cachedMessage).listen((response) {
        String responseBody = response.body.trim();
        if (responseBody.startsWith('data: ')) {
          responseBody = responseBody.substring(6);
        }
        ...
        try {
          ...
          if (data.containsKey('choices') && data['choices'].isNotEmpty) {
            ...
            if (content.isNotEmpty) {
              add(ChatNewContentGeneratedEvent(content: content));
            }
          }
        } catch (e) {
      ...
      print('Error in chatNewPromptEvent: $e');
      emit(ChatNewMessageGeneratingErrorState());
    }
  }
  ...
  FutureOr<void> chatNewContentGeneratedEvent(
    ...
    if (cachedMessage.isNotEmpty && cachedMessage.last.role == 'assistant') {
      String currentContent = cachedMessage.last.content;
      cachedMessage.last = ChatMessageModel(
          role: 'assistant', content: currentContent + event.content);
    } else {
      cachedMessage
          .add(ChatMessageModel(role: 'assistant', content: event.content));
    }
    emit(ChatNewMessageGeneratedState());
  }
}
...
// for (String line in response.body.split('\n')) {
// The response body (likely a string of data) is split into lines using the newline (\n) character.
// This loop iterates over each line of the response, which can be useful when dealing with server-sent events (SSE) or streamed data.
// emit(ChatNewMessageGeneratingErrorState());
//If an error occurs, the bloc emits the ChatNewMessageGeneratingErrorState to indicate that there was an issue while generating a new message.
