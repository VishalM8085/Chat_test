...
Stream<http.Response> getChatGptResponseRepo(
  ...
  try {
    List<Map> mappedMessage = messages.map((msg) => msg.toMap()).toList();
    
    final uri = Uri.parse("https://chatflowai.onrender.com/api/v1/generate_response");
    ...
    await for (var chunk in response.stream.transform(utf8.decoder)) {
      ...
      if (chunk.trim().isNotEmpty) {
        yield http.Response(chunk, response.statusCode, headers: response.headers);
      }
    }
  } catch (e) {
    print('Error in getChatGptResponseRepo: $e');
    yield http.Response('Error: $e', 500);
  }
}
// Stream<http.Response> getChatGptResponseRepo(
//     List<ChatMessageModel> messages) async* {
